...
function request() {
	...
	}else if($("input[name=register_name]").val() == "") {
		alert("사업주명을 입력하세요.");
		$("input[name=register_name]").focus();
	} else if($("input[name=business_phone1]").val() == "") {
		alert("사업자  휴대폰번호를 입력하세요.");
		$("input[name=business_phone1]").focus();
	} else if($("input[name=business_phone2]").val() == "") {
		alert("사업자  휴대폰번호를 입력하세요.");
		$("input[name=business_phone2]").focus();
	} else if($("input[name=business_phone3]").val() == "") {
		alert("사업자  휴대폰번호를 입력하세요.");
		$("input[name=business_phone3]").focus();
	} else if($("input[name=restaurant_name]").val() == "") {
		alert("음식점 이름을 입력하세요.");
		$("input[name=restaurant_name]").focus();
	} else if($("input[name=restaurant_phone1]").val() == "") {
		alert("음식점 전화번호를 입력하세요.");
		$("input[name=restaurant_phone1]").focus();
	} else if($("input[name=restaurant_phone2]").val() == "") {
		alert("음식점 전화번호를 입력하세요.");
		$("input[name=restaurant_phone2]").focus();
	} else if($("input[name=restaurant_phone3]").val() == "") {
		alert("음식점 전화번호를 입력하세요.");
		$("input[name=restaurant_phone3]").focus();
	} else if($("input[name=restaurant_address1]").val() == "") {
		alert("음식점 주소(우편번호)를 입력하세요.");
		$("input[name=restaurant_address1]").focus();
	} else if($("input[name=restaurant_address2]").val() == "") {
		alert("음식점 주소(도로명주소)를 입력하세요.");
		$("input[name=restaurant_address2]").focus();
	} else if($("input[name=restaurant_address3]").val() == "") {
		alert("음식점 주소(지번주소)를 입력하세요.");
		$("input[name=restaurant_address3]").focus();
	} else if($("input[name=restaurant_address4]").val() == "") {
		alert("음식점 주소(상세주소)를 입력하세요.");
		$("input[name=restaurant_address4]").focus();
	} else if(!$("#box").is(":checked")) {
		alert("업종카데고리를 선택하세요");
		$("input[name=delivery_option]").focus();
	} else if($("input[id=somename]").val() == "") {
		alert("썸네일 등록하세요.");
		$("input[id=somename]").focus();
	} else if(!$("#yak").is(":checked")) {
...
<script>
    ...
    function sample4_execDaumPostcode() {
        new daum.Postcode({
            oncomplete: function(data) {
                ...
                if(data.autoRoadAddress) {
                    ...
                    document.getElementById('guide').innerHTML = '';
                        ...
                        <p class="sb-addr"><input class="address4" type="text" class="w-xl" placeholder="상세주소" name="restaurant_address4"></p>
                    </td>
                </tr>
                <tr>
                    <th>업종 카테고리 <em>*</em></th>
                    <td>
                        <div class="checkbox-style">
                       			<input type="checkbox" name="box" id="box" value="프랜차이즈" />프랜차이즈
								<input type="checkbox" name="box" id="box" value="치킨" />치킨
								<input type="checkbox" name="box" id="box" value="피자양식" />피자/양식
								<input type="checkbox" name="box" id="box" value="중국집" />중국집
								<input type="checkbox" name="box" id="box" value="한식" />한식
								<input type="checkbox" name="box" id="box" value="일식/돈까스" />일식/돈까스<br>
								<input type="checkbox" name="box" id="box" value="족발보쌈"/>족발/보쌈
                        		<input type="checkbox" name="box" id="box" value="야식"/>야식
                        		<input type="checkbox" name="box" id="box" value="분식"/>분식
                        		<input type="checkbox" name="box" id="box" value="카페/디저트"/>카페/디저트
                        </div>
                    </td>
                </tr>
                <tr>
            ...
            <p class="chk-item">
                <label>
                    <input type="checkbox" name="yak" id="yak" value="y" checked="checked"> <strong>개인정보 수집 및 이용에 동의합니다.(필수)</strong>
                </label>
            </p>

            <div class="btn-set1">
                <button type="button" class="btn-type1" name="save_button" onclick="javascript:request()">입점 신청 완료</button>
